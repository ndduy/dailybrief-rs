...
## Current milestone
**R0** (`SPEC.md` §8): build M1 in full from the spec — config, migrations, ingest, candidates, the nine-tool rmcp server, the `claude-code` adapter, runner, scheduler, web pages, Access JWT middleware, CLI, Docker. Milestone spec goes to `spec/r0.md` (written by `/spec`), quality bar to `CONSTRAINTS.md`; module order for `/plan` is `SPEC.md` §7b. Nothing is built yet (`src/main.rs` is hello-world).
