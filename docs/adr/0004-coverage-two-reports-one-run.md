...
## Context
`CONSTRAINTS.md` sets two line-coverage floors: ≥ 80 % for `core::`, `mcp::`, `db::`, `harness::` (and `editor::`, the contract types), and ≥ 60 % for `web::`, `config::`, `commands::`. `cargo llvm-cov --fail-under-lines` applies one number to whatever files are in the report; it has no per-module floors. Running the suite twice to get two reports doubles the check budget.
...
cargo llvm-cov report --fail-under-lines 60 --ignore-filename-regex '/src/(core|mcp|db|harness|editor)/|/src/main\.rs|/src/bin/'
```
